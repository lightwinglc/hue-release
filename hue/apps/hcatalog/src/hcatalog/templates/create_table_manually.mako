...
<div class="span3">
    <div class="well sidebar-nav">
        <ul class="nav nav-list">
            ...
            <li><a href="${ url(app_name + ':create_table')}">${_('Create a new table manually')}</a>
            </li>
        </ul>
    </div>
</div>
...
<div id="step1" class="stepDetails">
    <fieldset>
        <div class="alert alert-info">
            <h3>${_('Create a table')}</h3>${_("Let's start with a name and description for where we'll store your data.")}
        </div>
...
<div class="form-actions">
    ...
    <input id="submit" type="submit" name="createTable" class="btn btn-primary hide" value="${_('Create table')}"/>
</div>
</form>
</div>
</div>
...
<%def name="render_column(form, is_partition_form=False)">
    <div class="cnt well">
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <input type="text" class="column" name="${form["column_name"].html_name | n}"
                       value="${form["column_name"].data or ''}" placeholder="${_('Column Name')}"/>
                <span class="help-inline error-inline hide">${_('This field is required. Spaces are not allowed.')}</span>
